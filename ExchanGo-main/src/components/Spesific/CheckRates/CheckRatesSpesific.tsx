...
import LocationDropdown from "../../../components/Phase1/HomePage/CheckRates/Location";
import CurrencyDropdown from "../../../components/Phase1/HomePage/CheckRates/Dropdowns";
import AmountInput from "../../../components/Phase1/HomePage/CheckRates/Amount";
...
interface CheckRatesSpesificProps {
  onCheckRates?: (params: {
    source: string;
    target: string;
    amount: string;
...
const CheckRatesSpesific: React.FC<CheckRatesSpesificProps> = ({ onCheckRates }) => {
  ...
  useEffect(() => {
    ...
    if (sourceParam) {
      const foundSource = currencies.find((c) => c.code === sourceParam);
      if (foundSource) {
        setSourceCurrency(foundSource);
      }
    }

    if (targetParam) {
      const foundTarget = currencies.find((c) => c.code === targetParam);
      if (foundTarget) {
        setTargetCurrency(foundTarget);
      }
    }
  ...
  const handleCheckRates = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const params = {
      ...
      amount: amount,
      location: selectedLocation?.name ?? "Casablanca",
      lat: selectedLocation?.coordinates?.lat,
      lng: selectedLocation?.coordinates?.lng,
    };

    // Update URL parameters instead of redirecting
    const currentParams = new URLSearchParams(window.location.search);
    currentParams.set("source", params.source);
    currentParams.set("target", params.target);
    currentParams.set("amount", params.amount);
    currentParams.set("city", params.location);
    
    if (params.lat && params.lng) {
      currentParams.set("lat", params.lat.toString());
      currentParams.set("lng", params.lng.toString());
    }
...
export default CheckRatesSpesific;
